...
产品的目标形态是**真人优先、AI 保底**：
- **真人优先**：两个此刻情绪相近的真人（情绪向量相似度 ≥ 阈值）会被**配成一对真实的匿名对话**——后端维护一个实时**等待池**，有相近的真人在等就立刻配对，并用轮询做双方**真实收发**（`app/livematch.py`，纯内存 + 单副本，无需数据库）。
- **AI 保底**：没有合适真人时，回退到由 Claude 扮演的情绪相近匿名对象（即下文/当前演示的主路径），保证任何时候进来都不会冷场。
- **为什么实机演示走 AI**：单个评测者无法同时触发两个真人配对，且赛题明确允许「mock 不同用户来演示对话」。但**该机制真实可用**——开两个浏览器标签、各写一段相近的心情，调用 `/api/live/*` 即可把两个真人配到一起、互相聊天（已端到端验证）。
- 接口：`POST /api/live/join`（加入池/即时配对）、`GET /api/live/status`（轮询配对状态）、`POST|GET /api/live/{id}/messages`（真人间收发）、`POST /api/live/leave`。

> 也就是说：当前线上以 **AI 保底**路径提供稳定、可单人体验的完整闭环；**真人匹配**作为后台机制已实现并可用，规模化只需把内存池换成共享存储（Redis）即可。
